...
export default function GuestAssistant() {
  ...
  // This is the site guide for people who aren't logged in yet — once a
  // vendor is signed in, DashboardAssistant takes over as their guide.
  ...
  const send = async (text) => {
    ...
    try {
      const res = await api.guestAssistantChat(trimmed, history);
      setMessages((prev) => [...prev, { role: "assistant", content: res.reply }]);
      if (res.navigate?.screen) {
        router.push(`/${res.navigate.screen}`);
      }
    } catch (err) {
      setError(err.message || "Couldn't reach the site guide.");
    } finally {
      setSending(false);
    }
  };
  ...
  return (
    <>
      <button
        ...
        title="Ask botimi"
      >
        <span className="material-symbols-outlined text-[26px]">{isOpen ? "close" : "chat_bubble"}</span>
      </button>
        ...
        <div className="fixed bottom-24 right-6 z-[999998] w-[360px] max-w-[calc(100vw-32px)] h-[520px] max-h-[calc(100vh-140px)] flex flex-col bg-surface-container border border-outline-variant rounded-2xl shadow-2xl overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-surface-container-high border-b border-outline-variant">
            <div className="flex items-center gap-2 min-w-0">
              <span className="material-symbols-outlined text-primary shrink-0">chat_bubble</span>
              <div className="min-w-0">
                <p className="font-label-md text-label-md text-on-surface font-bold truncate">botimi</p>
                <p className="text-[11px] text-on-surface-variant truncate">Ask about pricing, features, getting started</p>
              </div>
            </div>
            <button onClick={() => setIsOpen(false)} className="text-on-surface-variant hover:text-on-surface shrink-0" aria-label="Close">
